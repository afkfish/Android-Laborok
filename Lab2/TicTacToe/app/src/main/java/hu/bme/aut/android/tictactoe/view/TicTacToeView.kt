...
class TicTacToeView: View {
    ...
    private fun drawGameArea(canvas: Canvas) {
        ...
        canvas.drawLine(0F, heightFloat / 3, widthFloat, heightFloat / 3, paintLine)
        canvas.drawLine(0F, heightFloat / 3 * 2, widthFloat, heightFloat / 3 * 2, paintLine)

        // two vertical lines
        canvas.drawLine(widthFloat / 3, 0F, widthFloat / 3, heightFloat, paintLine)
        canvas.drawLine(widthFloat / 3 * 2, 0F, widthFloat / 3 * 2, heightFloat, paintLine)
        ...
        when (event?.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (tX < 3 && tY < 3 && TicTacToeModel.getFieldContent(tX, tY) == TicTacToeModel.EMPTY) {
                    ...
                    invalidate()
                    val result = TicTacToeModel.checkWinner()
